...
%    a Winkler foundation with 1, 2, 4, 8 elements using a quadratic basis.
...
%    
%    demo3_driver_1dbar_p2
%    
%--------------------------------------------------------------------------
function demo3_driver_1dbar_p2()
    clc;
    close all;
    ...
    for i = 1 : length(problem_size)
        ...
        %------------------------------------------------------------------
        analyze_1dbar_p2(problem_size{i});
        ...
        %  solution files
        %------------------------------------------------------------------
        load(strcat('./demo3_analyze_1dbar_winkler/assembly_files/assembly_1dbar_p2_numElements', problem_size{i}), 'nodes', 'elements');
        load(strcat('./demo3_analyze_1dbar_winkler/solution_files/solution_1dbar_p2_numElements', problem_size{i}), 'u', 'gcon');
        ...
        for e = 1 : size(elements, 1)
            ...
            h_markers(i, e) = ...
            plot([x1 x3], [u_h(1) u_h(end)], 's', 'Color', [0 0 0], 'MarkerFaceColor', problem_color(i, :), 'MarkerSize', 10);
            ...
            h_markers(i + 4, e) = ...
            plot([x1 x3], [f_h(1) f_h(end)], 's', 'Color', [0 0 0], 'MarkerFaceColor', problem_color(i, :), 'MarkerSize', 10);
        ...
        title('Displacement (p = 2)', 'FontSize', 60);
        xlabel('x (m)'           , 'FontSize', 48);
        ylabel('u_h (\mum)      ', 'FontSize', 48, 'Rotation', 0);
        legend(h(1:i), problem_legend{1:i}, 'FontSize', 32, 'Location', 'SouthWest');
        axis([0 4 0e-5 2e-5]);
        axis square;
        grid on;
        set(gca, 'FontSize', 26, ...
        ...
        title('Internal force (p = 2)', 'FontSize', 60);
        xlabel('x (m)'         , 'FontSize', 48);
        ylabel('F_h (MN)      ', 'FontSize', 48, 'Rotation', 0);
        legend(h((1:i) + 4), problem_legend{1:i}, 'FontSize', 32, 'Location', 'SouthWest');
...
%    print('-dpng' , '-r300', 'plot_fields_p2.png');
end